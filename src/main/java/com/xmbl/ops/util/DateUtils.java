...
public class DateUtils {
	...
	*/
	...
	public static void main(String[] args) {
		long secondTime = getSecondTime("11:35:36");
	...
	public static long getJavaCurrentTimeMillisByCCurrentTimeMillis(long cCurrentTimeMillis) {
		...
		if (dateDifference > 0) {
			return (dateDifference / 10000l) - 8*3600*1000L;
		}
		return 0;
	}
	...
	public static Long getCCurrentTimeMillisByJavaCurrentTimeMillis(long javaCurrentTimeMillis) {
		try {
			javaCurrentTimeMillis +=  8*3600*1000L;
	...
	/**
	 * String --> date --> long(java) --> long(C#) 
	 * 根据java 当前时间毫秒数获取c#当前时间毫秒数
	 * @param javaCurrentTimeMillis
	 * @return
	 */
	public static Long getCCurrentTimeMillisByJavaString(String javaDateStr) {
		try {
			Date currentDate = DateToolUtils.parse(javaDateStr, DateToolUtils.FORMAT_LONG);
			Long time = currentDate.getTime();
			Long cCurrentTimeMillisByJavaCurrentTimeMillis = DateUtils.getCCurrentTimeMillisByJavaCurrentTimeMillis(time);
			return cCurrentTimeMillisByJavaCurrentTimeMillis;
		} catch (Exception e) {
			return null;
		}
	}
...
//			e.printStackTrace();
	...
	/**
	 * <span>根据时长秒时间获取当前时间的字符串格式</span>
	 * <br></br>
	 * 获取的时间字符串格式如下:
	 * xx年xx月xx日  xx时 xx分 xx秒
	 * 
	 * @param dateTimeCnt 时长 单位秒
	 * @return
	 */
	public static String getDateTimeStr(Long dateTimeCnt) {
		Long year = dateTimeCnt/(60*60*24*31*12);
		Long month = dateTimeCnt%(60*60*24*31*12)/(60*60*24*31);
		Long day = dateTimeCnt%(60*60*24*31)/(60*60*24);
		Long hours = dateTimeCnt%60*60*24/(60*60);
		Long minutes = dateTimeCnt%(60*60)/60;
		Long seconds = dateTimeCnt%60;
		return year +"年 " +month + "月 " + day +"日  " + hours + "时 " + minutes + "分 " + seconds + "秒";
	}
	
//	public static void main(String[] args) {
//		String dateTimeStr = getDateTimeStr(1200003l);
